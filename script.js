/* ══════════════════════════════════════════════════════════
   ...
   Features: Search, AI (Claude API), Dashboard, Analytics
══════════════════════════════════════════════════════════ */
...
function showSection(name) {
  ...
  document.querySelectorAll('.nav-pill').forEach(p => {
    if (p.textContent.toLowerCase().includes(name === 'ai' ? 'ai' : name === 'dashboard' ? 'dash' : 'search'))
...
function setFilter(el, industry) {
  ...
  // If search is active re-run, else browse
...
// ── ML-Style Scoring: Similarity function (cosine-like keyword TF match) ─────
...
function doSearch() {
  ...
  // Tokenize — basic NLP: split, lowercase, remove stop words
...
function buildBarChart() {
  // Average green score per industry
  const byIndustry = {};
  REACTIONS_DB.forEach(r => {
...
function buildHeatmap() {
  ...
  const colors = {
    'CRITICAL': '#f04040', 'POOR': '#f5a623', 'MODERATE': '#4eb8f0', 'GOOD': '#22d9a0'
  };
...
async function sendAI() {
  ...
  // Build context from DB (summarized for system prompt)
...
Keep responses focused, scientific, and practical for industrial decision-makers.`;
    ...
    removeTyping(typingId);
    appendChatMsg(
      `Unable to connect to AI service. Please ensure you're accessing this via a server with the Anthropic API key configured.\n\n**Tip:** You can still use the Search and Dashboard sections — they work fully offline.\n\n_Error: ${err.message}_`,
      'assistant', true
    );
  }
